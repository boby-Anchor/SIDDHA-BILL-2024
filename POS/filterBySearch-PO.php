    ...
    foreach ($userLoginData as $userData) {
        ...
        if (!empty($bnInput)) {
            ...
            $tableRowCount = 1;
             ...
             $p_medicine_rs = $conn->query("SELECT producttoshop.*, p_brand.name AS brand,
                              ...
                              WHERE producttoshop.shop_id = '$shop_id' AND productToShopStatus = 'added' AND p_medicine.code LIKE '%$bnInput%' ORDER BY stock2.stock_id ASC");
        ...
        if (!empty($pcInput)) {
            ...
            $p_medicine_rs = $conn->query("SELECT producttoshop.*, p_brand.name AS brand,
                              ...
                              INNER JOIN stock2 ON p_medicine.code = stock2.stock_item_code
                              INNER JOIN p_brand ON p_medicine.brand = p_brand.id
                              WHERE producttoshop.shop_id = '$shop_id' AND productToShopStatus = 'added' AND p_medicine.name LIKE  '%$pnInput%'  ORDER BY stock2.stock_id ASC");
