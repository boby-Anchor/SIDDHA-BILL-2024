                                    ...
                                    <table class="table table-bordered table-hover">
                                        <thead>
                                            <tr class="bg-info">
                                                ...
                                                <th class="adThText">INVOICE Added Date</th>
                                        ...
                                        <tbody>
                                            ...
                                            if (isset($_SESSION['store_id'])) {
                                                ...
                                                foreach ($userLoginData as $userData) {
                                                    ...
                                                    $grn_details_result = $conn->query("SELECT * FROM `invoices` WHERE shop_id = '$shop_id'");

                                            ?>
                                                    <?php while ($grn_details_data = $grn_details_result->fetch_assoc()) { ?>
                                                        <tr>
                                                            <td><?= $grn_details_data["invoice_id"] ?></td>
                                                            <td>
                                                                <?php
                                                                $itemCount_result = $conn->query("SELECT COUNT(invoiceNumber) AS invoiceCount FROM invoiceitems WHERE invoiceitems.invoiceNumber = '" . $grn_details_data["invoice_id"] . "'");
                                                                $itemCount_data = $itemCount_result->fetch_assoc();
                                                                ?>
                                                                <button class="btn dropdown-toggle badge badge-info " type="button" data-bs-toggle="dropdown" aria-expanded="false" data-bs-placement="bottom-start"> <?= $itemCount_data['invoiceCount'] ?> </button>
                                                                <ul class="dropdown-menu">
                                                                    <table class="table" id="poItemsTable<?= $grn_details_data["invoice_id"] ?>">
                                                                        <thead>
                                                                            <tr>
                                                                                <th scope="col">#</th>
                                                                        ...
                                                                        <tbody>
                                                                            <?php
                                                                            $poItems_result = $conn->query("SELECT * FROM invoiceitems INNER JOIN p_medicine ON invoiceitems.invoiceItem = p_medicine.name WHERE invoiceitems.invoiceNumber = '" . $grn_details_data["invoice_id"] . "'");
                                                                            while ($poItems_data = $poItems_result->fetch_array()) { ?>
                                                                                <tr>
                                                                                    ...
                                                                                    <td><?= $poItems_data["invoiceItem_price"] ?>.00</td>
                                                                                </tr>
                                                                            <?php } ?>
                                                                        </tbody>
                                                                    </table>
                                                                    <button class="btn btn-warning" style="font-weight: bold; font-family: 'Source Sans Pro';" onclick="printTable('<?= $grn_details_data['invoice_id'] ?>');"> <i class="nav-icon fas fa-copy"></i> PRINT</button>
                                                                </ul>
                                                            </td>
                                                            <td><?= $grn_details_data["created"] ?></td>
                                                            <td><?= $grn_details_data["total_amount"] ?>.00</td>
                                                        </tr>
                                                    <?php } ?>
                                            <?php
                                                }
